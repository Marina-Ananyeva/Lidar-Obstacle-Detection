...
struct Lidar
{
    ...
    {
        // set minDistance to remove points from roof of ego car
        minDistance = 5;
        maxDistance = 50;
        resoultion = 0.2;
        // set sderr to get more interesting pcd files
        sderr = 0.2;
        cars = setCars;
        groundSlope = setGroundSlope;

        // increase number of layers to get higher resoultion pcd
        int numLayers = 8;
        // the steepest vertical angle
        double steepestAngle =  30.0*(-pi/180);
        double angleRange = 26.0*(pi/180);
        // set to angle increment to get higher resoultion pcd
